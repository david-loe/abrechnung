<template>
  <ListElement
    ...
    <template #item-reference="{reference}">
      <RefStringBadge :number="reference" type="Travel" :show-copy="false" />
    </template>
    ...
    <template #item-startDate="{ startDate }">{{ formatter.simpleDate(startDate) }}</template>
    <template #item-destinationPlace="{ destinationPlace }">
      <PlaceElement :place="destinationPlace" />
    </template>
    ...
    <template #item-state="{ progress, state }">
      <StateBadge :state="state" :StateEnum="TravelState" style="display: inline-block" />
      <ProgressCircle class="ms-3" v-if="state === TravelState.APPROVED" :progress="progress" style="display: inline-block" />
    </template>
...
import { refStringRegexLax, TravelSimple, TravelState, travelStates } from 'abrechnung-common/types.js'
...
if (window.innerWidth > bp.md) {
  headers.push(
    { text: 'labels.destinationPlace', value: 'destinationPlace' },
    { text: 'labels.startDate', value: 'startDate', sortable: true },
    ...
    { text: 'labels.traveler', value: 'owner' },
